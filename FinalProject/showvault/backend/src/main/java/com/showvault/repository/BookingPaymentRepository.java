...
public interface BookingPaymentRepository extends JpaRepository<BookingPayment, Long> {
    ...
    List<BookingPayment> findByStatus(BookingPayment.PaymentStatus status);
    
    List<BookingPayment> findByMethod(BookingPayment.PaymentMethod method);
    ...
    Optional<BookingPayment> findByBookingIdAndStatus(Long bookingId, BookingPayment.PaymentStatus status);
}