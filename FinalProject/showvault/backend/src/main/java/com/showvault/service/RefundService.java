...
public class RefundService {
    ...
    public BookingPayment processRefund(Long bookingId) {
        ...
        Optional<BookingPayment> originalPaymentOpt = paymentRepository.findByBookingIdAndStatus(
                bookingId, BookingPayment.PaymentStatus.COMPLETED);
        ...
        refundPayment.setStatus(BookingPayment.PaymentStatus.PENDING);
        ...
        if (refundProcessed) {
            refundPayment.setStatus(BookingPayment.PaymentStatus.COMPLETED);
            ...
            // Send notification
            notificationService.sendRefundNotification(booking, refundAmount.toString());
        } else {
            refundPayment.setStatus(BookingPayment.PaymentStatus.FAILED);
